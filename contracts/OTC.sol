...
contract OTC is Ownable {
    ...
    // token address
    address public base; // define the base address
    address public quote; // define the quote address (the trading token in exchange for the base)
    ...
    uint public PRICE_DECIMALS = 1000;

    constructor (address _base, address _quote, uint _totalSellFees, uint _toBuyerRatioOnSellFees) {
    ...
    *   @notice _quoteAmount is the amount of base token the user want to trade for base 
    ...
    */
    function createBuyOrder(uint _quoteAmount, uint _maxBuyPrice, uint _lastId, uint _orderIteration) external lastIdValid(0, _maxBuyPrice, _lastId) returns (bool) {
        ...
        IERC20(quote).safeTransferFrom(msg.sender, address(this), _quoteAmount);
        ...
        AskOrderBook[newAskId] = AskOrder(newAskId, true, msg.sender, _quoteAmount, _maxBuyPrice, _lastId, AskOrderBook[_lastId].nextOrderId); // (AskOrderBook[_lastId].nextOrderId, note) : set the next order id of the new order to the next order id of the last id order (pfiouuuuu ..)
            ...
            if (checkBoxPrice(0, _maxBuyPrice)) { // if there is a sell order with its min sell price under the max buy price, an exchange can proceed
                for (uint i = 0; i < _orderIteration; i++) { // for _orderIteration, continue to execute transactions until the buy amount is 0 or there are no more sell orders in touch with this buy order price
                    if (topBidOrder != 0) {
                        ...
                        if (AskOrderBook[newAskId].quoteAmount == 0) { // if this buy order is entierly filled (quoteAmount = 0), we can exit the sell order search
    ...
    *   @notice _orderIteration is the number of sell order this order will iterate until order completion
    */
    function createSellOrder(uint _baseAmount, uint _minSellPrice, uint _lastId, uint _orderIteration) external lastIdValid(1, _minSellPrice, _lastId) returns (bool) {
        require(_orderIteration > 0);
        ...
        IERC20(base).safeTransferFrom(msg.sender, address(this), _baseAmount);
        ...
        // create the new buy order 
        BidOrderBook[newBidId] = BidOrder(newBidId, true, msg.sender, _baseAmount, _minSellPrice, _lastId, BidOrderBook[_lastId].nextOrderId); // (BidOrderBook[_lastId].nextOrderId, note) : set the next order id of the new order to the next order id of the last id order (pfiouuuuu ..)
            ...
            if (checkBoxPrice(1, _minSellPrice)) { // if there is a sell order with its min sell price under the max buy price, an exchange can proceed
                for (uint i = 0; i < _orderIteration; i++) { // for _orderIteration, continue to execute transactions until the buy amount is 0 or there are no more sell orders in touch with this buy order price
                    if (topAskOrder != 0) {
                        ...
                        if (BidOrderBook[newBidId].baseAmount == 0) { // if this buy order is entierly filled (quoteAmount = 0), we can exit the sell order search
    ...
    function removeOrder(uint8 _side, uint _orderId) private {
        if (_side == 0) {
            ...
            IERC20(quote).safeTransfer(msg.sender, order.quoteAmount);
            ...
            IERC20(base).safeTransfer(msg.sender, order.baseAmount);
    ...
    function cancelOrder(uint8 _side, uint _orderId) external {
        ...
        if (_side == 0) {
            ...
            require(AskOrderBook[_orderId].quoteAmount > 0);

            removeOrder(0, _orderId);
        } 
        else {
            require(BidOrderBook[_orderId].owner == msg.sender);
            require(BidOrderBook[_orderId].baseAmount > 0);
    ...
    function executeTransaction(uint _buyOrderId, uint _sellOrderId) private {
        ...
        uint txFees = sellOrder.baseAmount.mul(totalSellFees).div(1000);
        uint baseAmountWithFees = sellOrder.baseAmount.sub(txFees); // amount of base the seller will be pay for (his real amount sub fees)
        uint baseAmount; // with fees
        uint quoteAmount;

        // get the limitants amounts of base and quote token
        if (baseAmountWithFees.mul(transactionPrice).div(PRICE_DECIMALS) < buyOrder.quoteAmount) { // limitant is base
            baseAmount = baseAmountWithFees;
            quoteAmount = baseAmount.mul(transactionPrice).div(PRICE_DECIMALS);

            removeOrder(1, _sellOrderId); // entierly remove the order filled
            buyOrder.quoteAmount = buyOrder.quoteAmount.sub(quoteAmount); // update the amount remaining after this execution
        } else if (baseAmountWithFees.mul(transactionPrice) > buyOrder.quoteAmount) { // limitant is quote
            quoteAmount = buyOrder.quoteAmount;

            uint _baseAmount = quoteAmount.div(transactionPrice).mul(PRICE_DECIMALS).div(uint(1000).sub(totalSellFees)).mul(1000); // compute the max base amount for quoteAmount
            txFees = _baseAmount.mul(totalSellFees).div(1000); // we need to re set txFees with the real fees because baseAmount is less than sellbaseAmount
            baseAmount = _baseAmount.sub(txFees); // base amount that will be send (with fees)

            removeOrder(0, _buyOrderId); // entierly remove the order filled
            sellOrder.baseAmount = sellOrder.baseAmount.sub(baseAmount); // update the amount remaining after this execution
        ...
        uint baseToTransferAmount = baseAmount.add(feesToBuyerAmount);

        IERC20(base).burn(feesToBurnAmount);
        IERC20(base).safeTransfer(buyOrder.owner, baseToTransferAmount);
        IERC20(quote).safeTransfer(sellOrder.owner, quoteAmount);
    }
}